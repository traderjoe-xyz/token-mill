...
contract TMStaking is ReentrancyGuardUpgradeable, ITMStaking {
    ...
    function getPendingRewards(address token, address account) external view override returns (uint256 pending) {
        ...
        if (totalShares > 0) {
            (, uint256 totalPendingReward) = ITMMarket(market).getPendingFees();
            uint256 accRewardPerShare = staking.accRewardPerShare + (totalPendingReward * PRECISION) / totalShares;
