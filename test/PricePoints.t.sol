...
contract PricePointsTest is Test {
    ...
    function test_getAmount() public {
...

        {
            ...
            (uint256 actualBaseAmount, uint256 quoteAmount) = _curve.getQuoteAmount(circSupply, base, false);
            (uint256 baseAmount, uint256 actualQuoteAmount) = _curve.getBaseAmountOut(circSupply, quoteAmount);
...

        {
            ...
            (uint256 actualBaseAmount, uint256 quoteAmount) = _curve.getQuoteAmount(circSupply, base, false);
            (uint256 baseAmount, uint256 actualQuoteAmount) = _curve.getBaseAmountOut(circSupply, quoteAmount + 1);
    ...
    function test_getDeltaAmount() public {
...

        {
            ...
            (int256 actualDeltaBaseAmount, int256 deltaQuoteAmount) = _curve.getDeltaQuoteAmount(circSupply, deltaBase);
            (int256 deltaBaseAmount, int256 actualDeltaQuoteAmount) =
                _curve.getDeltaBaseAmount(circSupply + uint256(-deltaBase), -deltaQuoteAmount);
...

        {
            ...
            (int256 actualDeltaBaseAmount, int256 deltaQuoteAmount) =
                _curve.getDeltaQuoteAmount(circSupply, deltaBase - 1);
            (int256 deltaBaseAmount, int256 actualDeltaQuoteAmount) =
                _curve.getDeltaBaseAmount(circSupply + uint256(-deltaBase), -(deltaQuoteAmount + 1));
    ...
    function test_Fuzz_getQuoteAmount(
        ...
        {
            (uint256 actualBaseAmount, uint256 quoteAmount) = _curve.getQuoteAmount(circSupply, base, false);
            (uint256 baseAmount,) = _curve.getBaseAmountOut(circSupply, quoteAmount);
        ...
        {
            (uint256 actualBaseAmount, uint256 quoteAmount) = _curve.getQuoteAmount(circSupply, base, true);
            (uint256 baseAmount,) = _curve.getBaseAmountIn(circSupply + actualBaseAmount, quoteAmount);
    ...
    function test_Fuzz_getBaseAmount(
        ...
        (, uint256 maxQuote) = _curve.getQuoteAmount(circSupply, _totalSupply - circSupply, true);
        quote = bound(quote, 0, maxQuote);

        {
            (uint256 baseAmount, uint256 actualQuoteAmount) = _curve.getBaseAmountOut(circSupply, quote);
            (, uint256 quoteAmount) = _curve.getQuoteAmount(circSupply, baseAmount, false);
        ...
        {
            (uint256 baseAmount, uint256 actualQuoteAmount) = _curve.getBaseAmountIn(circSupply, quote);
            (, uint256 quoteAmount) = _curve.getQuoteAmount(circSupply - baseAmount, baseAmount, true);
    ...
    function test_Fuzz_getDeltaQuoteAmount(
        ...
        (int256 actualDeltaBaseAmount, int256 deltaQuoteAmount) = _curve.getDeltaQuoteAmount(circSupply, deltaBase);
        (int256 deltaBaseAmount, int256 actualDeltaQuoteAmount) =
            _curve.getDeltaBaseAmount(uint256(int256(circSupply) - actualDeltaBaseAmount), -deltaQuoteAmount);
    ...
    function test_Fuzz_getDeltaBaseAmount(
        ...
        (, uint256 maxQuoteIn) = _curve.getQuoteAmount(circSupply, _totalSupply - circSupply, true);
        (, uint256 maxQuoteOut) = _curve.getQuoteAmount(0, circSupply, true);

        deltaQuote = bound(deltaQuote, -int256(maxQuoteOut), int256(maxQuoteIn));

        (int256 deltaBaseAmount, int256 actualDeltaQuoteAmount) = _curve.getDeltaBaseAmount(circSupply, deltaQuote);
        (int256 actualDeltaBaseAmount, int256 deltaQuoteAmount) =
            _curve.getDeltaQuoteAmount(uint256(int256(circSupply) - deltaBaseAmount), -deltaBaseAmount);
    ...
    function test_Fuzz_VerifyConvexity(
        ...
        (, uint256 maxQuoteIn) = _curve.getQuoteAmount(circSupply, maxBaseOut, false);
        (, uint256 maxQuoteOut) = _curve.getQuoteAmount(0, maxBaseIn, false);

        {
            int256 deltaBase0 = bound(delta0, -int256(maxBaseOut), 0);
            int256 deltaBase1 = bound(delta1, -int256(maxBaseOut) - deltaBase0, 0);

            (, int256 deltaQuote0) = _curve.getDeltaQuoteAmount(circSupply, deltaBase0);
            (, int256 deltaQuote1) = _curve.getDeltaQuoteAmount(uint256(int256(circSupply) - deltaBase0), deltaBase1);
            (, int256 deltaQuote01) = _curve.getDeltaQuoteAmount(circSupply, deltaBase0 + deltaBase1);
...

        {
            ...
            (int256 deltaBase0,) = _curve.getDeltaBaseAmount(circSupply, deltaQuote0);
            (int256 deltaBase1,) = _curve.getDeltaBaseAmount(uint256(int256(circSupply) - deltaBase0), deltaQuote1);
            (int256 deltaBase01,) = _curve.getDeltaBaseAmount(circSupply, deltaQuote0 + deltaQuote1);
    ...
    function test_Fuzz_getDeltaQuoteAmountSplitFirstSwap(
        ...
        // deltaBase >= 0
        {
            ...
            (int256 actualDeltaBaseAmount0, int256 deltaQuoteAmount0) =
                _curve.getDeltaQuoteAmount(circSupply, deltaBase0);
            (int256 actualDeltaBaseAmount1, int256 deltaQuoteAmount1) =
                _curve.getDeltaQuoteAmount(uint256(int256(circSupply) - actualDeltaBaseAmount0), deltaBase1);

            (int256 deltaBaseAmount01, int256 actualDeltaQuoteAmount01) = _curve.getDeltaBaseAmount(
    ...
    function test_Fuzz_getDeltaQuoteAmountSplitSecondSwap(
        ...
        // deltaBase >= 0
        {
            ...
            (int256 deltaBaseAmount1, int256 actualDeltaQuoteAmount1) =
                _curve.getDeltaBaseAmount(uint256(int256(circSupply) - actualDeltaBaseAmount0), -deltaQuoteAmount1);
            (int256 deltaBaseAmount2, int256 actualDeltaQuoteAmount2) = _curve.getDeltaBaseAmount(
    ...
    function test_Fuzz_getDeltaBaseAmountSplitFirstSwap(
        ...
        // deltaQuote >= 0
        {
            (, uint256 maxQuoteIn) = _curve.getQuoteAmount(circSupply, _totalSupply - circSupply, false);

            int256 deltaQuote0 = bound(delta0, 0, int256(maxQuoteIn));
            int256 deltaQuote1 = bound(delta1, 0, int256(maxQuoteIn) - deltaQuote0);

            (int256 deltaBaseAmount0, int256 actualDeltaQuoteAmount0) =
                _curve.getDeltaBaseAmount(circSupply, deltaQuote0);
            (int256 deltaBaseAmount1, int256 actualDeltaQuoteAmount1) =
                _curve.getDeltaBaseAmount(uint256(int256(circSupply) - deltaBaseAmount0), deltaQuote1);

            (int256 actualDeltaBaseAmount01, int256 deltaQuoteAmount01) = _curve.getDeltaQuoteAmount(
        ...
        // deltaQuote <= 0
        {
            (, uint256 maxQuoteOut) = _curve.getQuoteAmount(0, circSupply, false);
    ...
    function test_Fuzz_getDeltaBaseAmountSplitSecondSwap(
        ...
        // deltaQuote >= 0
        {
            ...
            (int256 actualDeltaBaseAmount1, int256 deltaQuoteAmount1) =
                _curve.getDeltaQuoteAmount(uint256(int256(circSupply) - deltaBaseAmount0), -deltaBaseAmount1);
            (int256 actualDeltaBaseAmount2, int256 deltaQuoteAmount2) = _curve.getDeltaQuoteAmount(
